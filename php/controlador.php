...
switch ($opcion) {
	...
	case 9 :
		...
		if (sizeof ( $datosPublicacion ) > 0) {
			...
			foreach ( $datosPublicacion as $publicaciones ) {
				...
				$dia1 = @date ( "d", strtotime ( $publicaciones [4] ) );
				$dia2 = @date ( "d", strtotime ( $publicaciones [10] ) );
				$year1 = @date ( "Y", strtotime ($publicaciones [4] ) );
				$year2 = @date ( "Y", strtotime ($publicaciones [10]  ) );
				$mes1 = substr ( @date ( "d-m-Y", strtotime ( $publicaciones [4] ) ), 3, 2 );
				$mes2 = substr ( @date ( "d-m-Y", strtotime ( $publicaciones [10] ) ), 3, 2 );
				...
				$cadena .= "<a href='blog/noticias_preview.php?idpublicacion=" . base64_encode($publicaciones [0]) . "'>" . $publicaciones [5] . "</a>";
				$cadena .= "</h2>";
				$cadena .= "<div class='art-postheadericons art-metadata-icons'>";
				$cadena .= "        <img src='images/iconos/calendar-7.png' width=45 height=45> ";
				$cadena .= "        Fecha de creacion:" .$cadenaFecha1;
				//$cadena .= "        Fecha de publicacion:" .$cadenaFecha2."<br>";
				...
				$cadena .= "</div>";
				$cadena .= "</div>";
				$cadena .= "<br><a  style ='text-decoration:none;color:#000034;font-weight:bold;' href='blog/noticias_preview.php?idpublicacion=" . base64_encode($publicaciones [0]) . "'>Leer M&aacute;s...</a>";
				$cadena .= "</div>";
				/**
				 * ***********FIN CONTENIDO**********************
				 */
	...
	case 12 :
		...
		//if($_SESSION['user_admin']==''||$_SESSION['user_docente']==null){
	...
	case 16 :
	    //Para evitar que se quiera entrar al script sin sesion o no requerido
		if($_SESSION['user_admin']==''||$_SESSION['user_admin']==null){
			//echo "<script>window.location='../inicio.php';</script>";
			
		   if ($_SESSION['matricula']==''||$_SESSION['matricula']==null) {
			header("Location:../inicio.php");
		   }
		   else{
	...
	case 18 :
		...
		if (sizeof ( $inscripcion ) > 0) {
			...
			$cadenaReporte .= "<div align=right>";
			$cadenaReporte .= "<h4 align='center' style='position:fixed;'><label class='label label-default'>SOLICITUDES ENCONTRADAS:" . sizeof ( $inscripcion ) . "</label></h4><br><br>";
			$cadenaReporte .= "<h4 align='center' style='position:fixed;'><label class='label label-default'>SOLICITUDES PENDIENTES:" . $numero . "</label></h4><br></div><br><br>";
			...
			$cadenaReporte .= "<th align='center' width='14%' class='smallText'>FECHA DE SOLICITUD </th>";
            ...
            //Fin
				...
				if ($reporte [10] == "proceso-re") {
					...
					$cadenaReporte .= "<br>";
					$cadenaReporte .= "<h4><a style='text-decoration:none;' href='../usuarios/alumnos/calificacion_alumno.php?matricula=" . $reporte [1] . "' target='_blank' ><img src='images/iconos/book.png' width=35 heigth=35>Ver calificaciones </a></h4><br>";
					$cadenaReporte .= "<h4>EN PROCESO DE REINSCRIPCI&Oacute;N</h4>";
					$cadenaReporte .= "<a href='#' onclick=\"javascript:aceptarReinscripcion('12','" . $reporte [1] . "');\"  style=text-decoration:none;>";
					$cadenaReporte .= "<img src=../images/iconos/stop.png width=45 heigth=45 style='border:none;'>";
					$cadenaReporte .= "VALIDAR SOLICITUD";
					$cadenaReporte .= "</a>";
				} else {
					if ($reporte [10] == "reinscrito") {
						$cadenaReporte .= "<a style='text-decoration:none;' href='asignar_materias.php?matricula=" . $reporte [1] . "'><img src='images/iconos/8_48x48.png' width=35 heigth=35>Asignar materias</a>";
						$cadenaReporte .= "<h4 style='color:blue;'>" . strtoupper ( $reporte [10] ) . "</h4>";
						$cadenaReporte .= "<br>";
						$cadenaReporte .= "<a href='#' onclick=\"javascript:cancelarReinscripcion('13','" . $reporte [1] . "');\" style='text-decoration:none;'>";
						$cadenaReporte .= "<img src='../images/iconos/tick.ico' width='45' heigth='45' style='border:none;'>";
			...
			$cadenaReporte .= "<h4> <img src='images/iconos/warning.ico' width=33 height=33><b>No se encontraron coindicencias</h4></b><br>";
			$cadenaReporte .= "<a style='text-decoration:none;' href='javascript:cargarReporte();'><b>Buscar de nuevo</b></a></td></tr>";
		}
		$cadenaReporte .= "</table>";
		echo $cadenaReporte;
		}
	...
	case 19 :
		...
		if($_SESSION['user_admin']==''||$_SESSION['user_admin']==null){
		   ...
		   if ($tablaInscripcionModificar->modificarInscripcion ( $inscripcion ) == "1") {
			  ...
			  echo "ERROR,CONSULTA CON AL ADMINISTRADOR";
		   }
	     }
		}
		else{
		...
		if (sizeof ( $datosPublicacion ) > 0) {
			foreach ( $datosPublicacion as $listaPublicaciones ) {
				 ...
				 */
				 ...
				 $cadena .= "<li><a style='text-decoration:none;color:#000034;' href='blog/noticias_preview.php?idpublicacion=" . base64_encode($listaPublicaciones [0]) . "'>";
	...
	case 33:
		...
		$sql = "SELECT id_publicacion,user,tipo_usu,cat_aviso,fecha_crea,titular,cont_publi,imagen_publi,archivo_publi FROM publicacion join usuario on publicacion.cve_usu=usuario.cve_usu join aviso on publicacion.id_aviso=aviso.id_aviso where month(fecha_crea)=month(curdate())  order by fecha_crea desc LIMIT 10";
		@mysql_query ( "SET NAMES 'utf8'" );
		$consulta = @mysql_query ($sql );
		$rows = @mysql_num_rows($consulta);

		if($rows>0){
			$cadenaLista.='<ol class="carousel-indicators hidden-xs">';
			for ($i=0; $i<$rows; $i++) {
				$noticias = @mysql_fetch_array($consulta);
				/**************FORMATOS DE FECHA**********************************************************/
				...
				/*******************************************************************************************/
				if ($i == 0) {
					$cadenaLista.='<li data-target="#carousel-example-generic" data-slide-to="0" class="active"></li>';
					$cadenaImagenes.='<div class="item active">';
				} else {
					$cadenaImagenes.='<div class="item">';
					$cadenaLista.='<li data-target="#carousel-example-generic" data-slide-to="' . $i . '"></li>';
				}
				$cadenaImagenes.="<a href='blog/noticias_preview.php?idpublicacion=".base64_encode($noticias[0])."'>";
				$cadenaImagenes.='<img style="width: 700px;height: 278px;" class="highlight" src="images/publicacion/'.$noticias[7].'" alt="">';
				$cadenaImagenes.='</a>';
				$cadenaImagenes.='<div class="carousel-caption">';
				$cadenaImagenes.='<h3><label class="label label-primary" style="font-weight:14px;text-transform:uppercase;">'.$noticias[5].' '.$cadenaFecha.'</label></h3>';
				$cadenaImagenes.='</div>';
				$cadenaImagenes.='</div>';
			}
			$cadenaLista.='</ol>';
			...
			$cadenaLista.='<ol class="carousel-indicators hidden-xs"><li data-target="#carousel-example-generic" data-slide-to="0" class="active"></li></ol>';
			$cadenaImagenes.='<div class="item active">';
			$cadenaImagenes.='<img style="width: 700px;height: 278px;" class="highlight" src="images/morelospreview.png" alt="">';
			$cadenaImagenes.='<br><br><br><br><div class="carousel-caption"><h3 align=center><label class="label label-primary" style="font-weight:14px;text-transform:uppercase;">No hay noticias disponibles!</label></h3></div>';
			$cadenaImagenes.='</div>';
		}

		$cadena.='<div id="noticias" class="carousel slide">
                        <!-- Indicators -->
                        '.$cadenaLista.'
                        <!--Indicators-->
                        ...
                        <!-- Controls -->
                        <a class="left carousel-control" href="#noticias" data-slide="prev">
                            <span class="icon-prev"></span>
                        </a>
                        <a class="right carousel-control" href="#noticias" data-slide="next">
