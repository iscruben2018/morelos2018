...
function confirmarRegistro(){
	...
	var respuesta=confirm("CONFIRMAS AGREGAR EL NUEVO CICLO:"+ciclo_actual+"? \nNOTA:AGREGALO CUANDO EL ANTERIOR HAYA TERMINADO");
	if(respuesta){
...
<body>
	...
	<form method='post' action='registrar_ciclo.php'
		...
		id='form_ciclo'>
		<label>Clave:</label>
		<input type='text' readonly name='cve_ciclo' id='cve_ciclo' value='<?php echo $claveCiclo+1;?>'>
		<br> 
		<label>Nombre:</label>
		<input type='text' value='<?php echo ($yearActual)."-".($yearSiguiente);?>'
		name='no_ciclo' id='no_ciclo' 
		placeholder="Sugerencia:<?php echo ($yearActual)."-".($yearSiguiente);?>"> 
		<br><label>Sugerencia:</label>
		<br><br>
